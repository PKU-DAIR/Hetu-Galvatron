...
def _reduce(input_, group=None):
    ...
    if get_tensor_model_parallel_world_size(group) == 1:
        return input_
    if group is None:
...
def _split_along_last_dim(input_, group=None):
    ...
    world_size = get_tensor_model_parallel_world_size(group)
    # Bypass the function if we are using only 1 GPU.
    if world_size == 1:
        return input_
    ...
    rank = get_tensor_model_parallel_rank(group)
...
def _reduce_scatter_along_last_dim(input_, group=None):
...
class _AllToAll_with_event(torch.autograd.Function):
    @staticmethod
    def forward(ctx, group, input, output_split_sizes, input_split_sizes, forward_event, backward_event, is_backward = False):
        """Forward function."""
        ctx.group = group
        ctx.output_split_sizes = output_split_sizes
        ctx.input_split_sizes = input_split_sizes
    ...
    def backward(ctx, *grad_output):
        ...
        return (
            None,
            _AllToAll_with_event.apply(ctx.group, *grad_output, ctx.input_split_sizes, ctx.output_split_sizes, None, ctx.backward_event, True),
            None,
            None,
            None,
            None,
...
def all_to_all_with_event(group, input_, output_split_sizes_=None, input_split_sizes=None, forward_event=None, backward_event=None):
    """Wrapper for autograd function"""
    return _AllToAll_with_event.apply(group, input_, output_split_sizes_, input_split_sizes, forward_event, backward_event, False)
