...
from galvatron.core.runtime.datasets.dummy_dataset import get_dummy_text_data_iterator, get_dummy_text_batch
import galvatron.core.runtime.parallel_state as parallel_state


def train(args:GalvatronRuntimeArgs):
    ...
    assert data_source == 'dummy', f"train_dist_varlen.py only supports dummy data, but got {data_source}"
    if (parallel_state.is_pipeline_first_stage() or parallel_state.is_pipeline_last_stage()) and parallel_state.get_vocab_tp_sp_rank() == 0:
        train_data_iterator = get_dummy_text_data_iterator(
    ...
    for iter_idx in range(getattr(args.train, "iteration", 0), args.train.train_iters):
        tokens, kwargs, loss_func = get_dummy_text_batch(train_data_iterator)
