...
CHECKPOINT_PATH=$BASE_DIR/checkpoints/laer_convergence/$MODEL_SIZE
...
if [ "$GALVATRON_PROFILE" = "1" ]; then
    ...
    ${LAUNCHER} ${TRAINER} ${MODEL_ARGS} ${TRAIN_ARGS} ${PARALLEL_ARGS} ${DATA_ARGS} ${CKPT_ARGS} | tee training_log/${METHOD}_convergence_${MODEL_SIZE}_${DATA}_batch${BATCH_SIZE}_seq${SEQUENCE_LENGTH}_aux${AUX}_${ARNOLD_ID}.log
fi