...
MODE="${MODE:-test}"
ALIGN_CONFIG="${ALIGN_CONFIG:-${SCRIPT_DIR}/train_dist.yaml}"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --mode)
      MODE="${2:-}"
...
# Unique suffix for this run (override with RUN_TAG=...). Do not repeat MODE here — WANDB_RUN_NAME adds it once.
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"

if [[ "${MODE}" == "baseline" ]]; then
  CURVE_STORE="${CURVE_STORE:-${SCRIPT_DIR}/loss_baseline${CONFIG_SUFFIX}.csv}"
else
  CURVE_STORE="${CURVE_STORE:-${SCRIPT_DIR}/loss_test${CONFIG_SUFFIX}.csv}"
fi
WANDB_PROJECT="${WANDB_PROJECT:-my-awesome-project}"
WANDB_ENTITY="${WANDB_ENTITY:-}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-${MODE}${CONFIG_SUFFIX}_${RUN_TAG}}"
...
TMP_DIR="$(mktemp -d "/tmp/accuracy_alignment_${MODE}_XXXXXX")"
