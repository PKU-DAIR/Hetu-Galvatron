...
@triton.jit
def _prepare_grad_all_to_all_send_kernel(
    # Input tensors
    padded_unsharded_grad_ptr,
    # Output tensors
    send_buffer_ptr,
    # Configuration
    world_size,
    ...
    """Prepare gradient send data for all_to_all"""
    rank = tl.program_id(0)
    expert_idx = tl.program_id(1)
    block_idx = tl.program_id(2)
    block_base_offset = block_idx * DATA_BLOCK_SIZE
    ...
    while shard_offset < DATA_BLOCK_SIZE:
        ...
        # Read specified column data from padded_unsharded_grad
        col_start = rank * expert_shard_size
        src_offset = (expert_idx * stride_input_expert + 
                     (col_start + block_offset) * stride_input_shard)
        data = tl.load(padded_unsharded_grad_ptr + src_offset, mask=mask)
...
def triton_all_to_all_backward(
    ...
    """Triton-optimized all_to_all backward pass
    ...
    _prepare_grad_all_to_all_send_kernel[grid](
        ...
        expert_shard_size,
        # Stride information
        world_size * expert_shard_size,  # stride_input_expert
        1,                                # stride_input_shard
        local_expert_num * expert_shard_size,  # stride_send_rank
        expert_shard_size,                # stride_send_expert
        1,                                # stride_send_shard
    )
    
    # 2. Execute all_to_all communication using megatron
    send_tensor = send_buffer.contiguous().view(-1)
    recv_tensor = all_to_all(process_group, send_tensor, 
                            [local_expert_num * expert_shard_size] * world_size,
                            [local_expert_num * expert_shard_size] * world_size)
    
    # Reshape back to buffer format for Triton kernel
    recv_buffer = recv_tensor.view(world_size, local_expert_num, expert_shard_size)
