        ...
        if self.sequence_parallel:
            ...
            for block in self.model_cur_stage:
                for m in block.modules():
                    ...
                    elif isinstance(m, FSDPModule): # fsdp2: carry sp_group on the param group for post_backward sp-norm reduce
                        fsdp2_state = m._get_fsdp_state()
                        if fsdp_param_group := fsdp2_state._fsdp_param_group:
                            fsdp_param_group.megatron_sp_group = self.layer_tp_groups[idx]
                idx += 1
