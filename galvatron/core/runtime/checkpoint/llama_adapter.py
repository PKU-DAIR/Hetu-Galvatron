...
def load_distributed_checkpoint(load:str, tp_groups: dist.ProcessGroup, name: str, submodule: nn.Module, module: nn.Module):
    ...
    args = get_args()
    load = os.path.join(load, f"iter_{args.ckpt.load_iteration}")
...
def _module_full_state_dict(m, is_fsdp2):
    ...
    NOTE: ``cpu_offload=True`` together with ``full_state_dict=True`` makes DCP set
    ``ranks_only=(0,)`` (see torch ``distributed/checkpoint/state_dict.py``), i.e. only
    global rank0 receives the gathered tensors and every other rank gets an empty dict.
    Because Megatron-TP shards differ per ``tp_rank``, every ``tp_rank`` must write its own
    full (DP-gathered) shard, so we gather with ``cpu_offload=False`` (``ranks_only=()`` ->
    all ranks) and move the tensors to CPU ourselves.
    """
    if is_fsdp2:
        state_dict = get_model_state_dict(m, options=StateDictOptions(full_state_dict=True, cpu_offload=False))
