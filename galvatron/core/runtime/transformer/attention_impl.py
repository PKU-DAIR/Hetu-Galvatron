...
class FlashSelfOrCrossAttention(torch.nn.Module):
    ...
    def forward(self, q, k, v, cu_seqlens_q=None, cu_seqlens_kv=None):
        ...
        if cu_seqlens_q is not None:
            assert cu_seqlens_kv is not None, "cu_seqlens_q and cu_seqlens_kv should be both provided or both None"
...
# --------- Shared helpers for varlen flash attention ----------

from flash_attn.flash_attn_interface import _flash_attn_varlen_forward, _flash_attn_varlen_backward

import inspect as _inspect
_varlen_fwd_params = _inspect.signature(_flash_attn_varlen_forward).parameters
_HAS_VARLEN_WINDOW_SIZE = 'window_size' in _varlen_fwd_params


def _varlen_fwd_kwargs(dropout_p, softmax_scale, causal, window_size, softcap, alibi_slopes, return_softmax):
    base = dict(dropout_p=dropout_p, softmax_scale=softmax_scale, causal=causal,
                softcap=softcap, alibi_slopes=alibi_slopes, return_softmax=return_softmax)
    if _HAS_VARLEN_WINDOW_SIZE:
        base['window_size'] = window_size
    else:
        base['window_size_left'] = window_size[0]
        base['window_size_right'] = window_size[1]
    return base


def _varlen_bwd_kwargs(dropout_p, softmax_scale, causal, window_size, softcap, alibi_slopes, deterministic, rng_state=None):
    base = dict(dropout_p=dropout_p, softmax_scale=softmax_scale, causal=causal,
                softcap=softcap, alibi_slopes=alibi_slopes, deterministic=deterministic,
                rng_state=rng_state)
    if _HAS_VARLEN_WINDOW_SIZE:
        base['window_size'] = window_size
    else:
...
def _call_varlen_fwd(*args, **kwargs):
    outputs = _flash_attn_varlen_forward(*args, **kwargs)
    if len(outputs) == 8:
        block_out, _, _, _, _, block_lse, _, _ = outputs
    else:
        block_out, block_lse, _, _ = outputs
...
# --------- Zigzag Ring Flash Attention with Variable-Length (Packed) Sequences ----------
# Reference: https://github.com/zhuzilin/ring-flash-attention/
# zigzag variant with global→local cu_seqlens conversion for CP.
...
def zigzag_ring_flash_attn_varlen_forward(
    ...
    assert causal == True, "zigzag ring is meaningless for causal=False"
    comm = RingComm(process_group)
    cp_size = comm.world_size

    # Compute local cu_seqlens and max_seqlen from global
    num_seqs = len(global_cu_seqlens) - 1
    local_cu_seqlens = torch.zeros(num_seqs + 1, dtype=global_cu_seqlens.dtype, device=global_cu_seqlens.device)
    for i in range(num_seqs):
        global_len = (global_cu_seqlens[i + 1] - global_cu_seqlens[i]).item()
        local_cu_seqlens[i + 1] = local_cu_seqlens[i] + global_len // cp_size
    local_max_seqlen = global_max_seqlen // cp_size

    half_index0 = get_half_index(local_cu_seqlens, front=True)
    half_index1 = get_half_index(local_cu_seqlens, front=False)
    half_cu_seqlens = local_cu_seqlens // 2
    half_max_seqlen = local_max_seqlen // 2
    ...
    def forward(q, k, v, causal):
        seqlen_q = q.shape[0]
        seqlen_kv = k.shape[0]
        cu_seqlens_q = half_cu_seqlens if seqlen_q == block_seq_len else local_cu_seqlens
        max_seqlen_q = half_max_seqlen if seqlen_q == block_seq_len else local_max_seqlen
        cu_seqlens_kv = half_cu_seqlens if seqlen_kv == block_seq_len else local_cu_seqlens
        max_seqlen_kv = half_max_seqlen if seqlen_kv == block_seq_len else local_max_seqlen
        return _call_varlen_fwd(
            q,
            k,
            v,
            cu_seqlens_q,
            ...
            max_seqlen_q,
            max_seqlen_kv,
            **_varlen_fwd_kwargs(dropout_p, softmax_scale, causal,
    ...
    for step in range(comm.world_size):
        ...
        if step == 0:
            block_out, block_lse = forward(q, k, v, causal=True)
            if lse_is_padded is None:
                lse_is_padded = block_lse.dim() == 3
            if lse_is_padded:
                block_lse = flatten_varlen_lse(block_lse, cu_seqlens=local_cu_seqlens)
            out, lse = update_out_and_lse(out, lse, block_out, block_lse)
        elif step <= comm.rank:
            k0 = k[half_index0]
            v0 = v[half_index0]
            ...
            block_out, block_lse = forward(q1, k, v, causal=False)
            if lse_is_padded:
                block_lse = flatten_varlen_lse(block_lse, cu_seqlens=half_cu_seqlens)
    ...
    out = out.to(q.dtype)
    if lse_is_padded:
        lse = unflatten_varlen_lse(lse, local_cu_seqlens, local_max_seqlen)
    else:
        lse = lse.squeeze(dim=-1).transpose(0, 1).contiguous()
    return out, lse
...
def zigzag_ring_flash_attn_varlen_backward(
    ...
    if softmax_lse.dim() == 3:
        softmax_lse1 = get_half_lse(softmax_lse, local_cu_seqlens, front=False)
    else:
        softmax_lse1 = softmax_lse[:, half_index1]
    block_seq_len = q.shape[0] // 2
    ...
    def backward(dout, q, k, v, out, softmax_lse, causal):
        ...
        _flash_attn_varlen_backward(
            ...
            max_seqlen_q,
            max_seqlen_kv,
            **_varlen_bwd_kwargs(dropout_p, softmax_scale, causal,
                                window_size, softcap, alibi_slopes,
...
class ZigzagRingFlashAttnVarlenFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        # Pre-compute local cu_seqlens and half_index for saving in ctx
        ...
        if is_half_index_tensor:
            ctx.save_for_backward(q, k, v, out, softmax_lse, global_cu_seqlens, half_index0, half_index1)
        else:
            ctx.save_for_backward(q, k, v, out, softmax_lse, global_cu_seqlens)
            ctx.half_index0 = half_index0
            ctx.half_index1 = half_index1

        ctx.global_max_seqlen = global_max_seqlen
        ctx.dropout_p = dropout_p
        ctx.softmax_scale = softmax_scale
        ctx.causal = causal
        ctx.window_size = window_size
    ...
    def backward(ctx, dout, *args):
        if ctx.is_half_index_tensor:
            q, k, v, out, softmax_lse, global_cu_seqlens, half_index0, half_index1 = ctx.saved_tensors
        else:
            q, k, v, out, softmax_lse, global_cu_seqlens = ctx.saved_tensors
...
class ZigzagRingFlashAttention(torch.nn.Module):
    ...
    def forward(self, q, k, v, cu_seqlens_q=None, cu_seqlens_kv=None):
        ...
        if cu_seqlens_q is not None:
            assert cu_seqlens_kv is not None, "cu_seqlens_kv must be provided when cu_seqlens_q is provided"
            assert torch.equal(cu_seqlens_q, cu_seqlens_kv), "cu_seqlens_q and cu_seqlens_kv must be identical"
            
            cu_seqlens = cu_seqlens_q
            # BSHD [B=1, S, H, D] -> THD [S, H, D]
            assert q.shape[0] == 1, f"varlen mode requires B=1, got B={q.shape[0]}"
            max_seqlen = (cu_seqlens[1:] - cu_seqlens[:-1]).max().item()
            q_ = q.squeeze(0)
            k_ = k.squeeze(0)
            v_ = v.squeeze(0)
            with torch.profiler.record_function("ZigZag_Ring_Flash_Attention_Varlen_Forward"):
                context = zigzag_ring_flash_attn_varlen_func(
                    q_, k_, v_,
                    global_cu_seqlens=cu_seqlens,
                    global_max_seqlen=max_seqlen,
                    dropout_p=self.attention_dropout,
                    softmax_scale=softmax_scale,
                    causal=self.causal,
                    group=self.cp_process_group,
                )
            return context.unsqueeze(0)  # [S, H, D] -> [B=1, S, H, D]
        else:
