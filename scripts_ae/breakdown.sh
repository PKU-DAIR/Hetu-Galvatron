...
if [ -d "$BASE_DIR/checkpoints/laer/$model_name" ]; then
    echo "Generating breakdown for $model_name with $approach..."
    cd $BASE_DIR/LAER-MoE/galvatron/models/moe
    mkdir -p $BASE_DIR/LAER-MoE/galvatron/models/moe/training_log/breakdown
    bash scripts/train.sh $model_name $layer_num $aux_loss $approach $dataset $capacity $batch_size $chunk $seq_length 1
    echo "Breakdown finished for $model_name"
else
