...
export default class MemoryCostModel {
    ...
    parseRawConfig() {
     ...
     // console.log("Parsing raw config data...");
      ...
      const memory_profiling = this.rawConfig.memory_profiling;
      const seqLengths = [];
      for (const key in memory_profiling) {
          if (key.match(/layertype_\d+(?:_sp)?$/)) {
              const layerKey = key;
              Object.keys(memory_profiling[layerKey]).forEach((seq) => {
      ...
      this.parameter_size = memory_profiling[layer_type][seqLengths[seqLengths.length - 1]].parameter_size; 
      this.tp_activation_per_bsz_dict = memory_profiling[layer_type][seqLengths[seqLengths.length - 1]].tp_activation_per_bsz_dict;
      ...
      this.other_parameter_size_pp_off = memory_profiling[other_layer_type_pp_off][seqLengths[seqLengths.length - 1]].model_states;
      this.other_parameter_size_pp_on_first = memory_profiling[other_layer_type_pp_on_first][seqLengths[seqLengths.length - 1]].model_states;
      this.other_parameter_size_pp_on_last = memory_profiling[other_layer_type_pp_on_last][seqLengths[seqLengths.length - 1]].model_states;
      ...
      this.other_activation_size_pp_off = memory_profiling[other_layer_type_pp_off][seqLengths[seqLengths.length - 1]].activation;
      this.other_activation_size_pp_on_first = memory_profiling[other_layer_type_pp_on_first][seqLengths[seqLengths.length - 1]].activation;
      this.other_activation_size_pp_on_last = memory_profiling[other_layer_type_pp_on_last][seqLengths[seqLengths.length - 1]].activation;
