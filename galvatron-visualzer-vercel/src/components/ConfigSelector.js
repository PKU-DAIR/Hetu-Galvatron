...
// State variables
// discarding original presets for timecost dev  
//   const [models] = useState(['qwen2.5-32b', 'llama3-8b', 'llama2-70b', 'llama3-70b']);
  const [models] = useState(['qwen2.5-32b']);
  ...
  const loadConfig = () => {
    ...
    setLoading(true);
    setError(null);
    // changed naming scheme to overall presets instead of `memory_profiling_x`
    // Build configuration file path (similar to memory_profiling_path in search_engine.py)
    const configName = `${selectedPrecision}_${selectedModel}.json`;
    
    // In Next.js, files in the public directory are served from the root
    fetch(`/configs/${configName}`)
      .then(response => {
        if (!response.ok) {
